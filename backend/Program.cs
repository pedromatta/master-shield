...
builder.Services.AddDbContext<DaedalaContext>(options => options.UseNpgsql(connectionString));
...
builder.Services
    ...
    .AddEntityFrameworkStores<DaedalaContext>()
...
builder.Services
    ...
    .AddCookie(options =>
    {
        options.Cookie.Name = "daedala_auth";
...
const string devCorsPolicy = "DaedalaDev";
...
{
    var database = scope.ServiceProvider.GetRequiredService<DaedalaContext>().Database;
    database.Migrate();
}
